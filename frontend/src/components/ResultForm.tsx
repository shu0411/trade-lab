...
export default function ResultForm({ entry, onSubmit }: Props) {
  ...
  const autoResult: Result | null =
    base && !isNaN(ep)
      ? ep > base ? 'success' : ep < base ? 'failure' : 'breakeven'
      : null;
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 bg-gray-50 rounded-xl p-4 border border-gray-200">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select
            value={result} onChange={(e) => setResult(e.target.value as Result)}
            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400"
          >
          ...
          {autoResult && result !== autoResult && (
            <p className="text-xs text-orange-500 mt-1">自動判定: {autoResult === 'success' ? '成功' : autoResult === 'failure' ? '失敗' : '引き分け'}</p>
          )}
        </div>
        <div>
          <label className="block text-xs font-medium text-gray-600 mb-1">最大含み益（%）</label>
          <input type="number" step="0.1" value={maxGainPct} onChange={(e) => setMaxGainPct(e.target.value)}
            placeholder="+7.2"
            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" />
        </div>
        <div>
          <label className="block text-xs font-medium text-gray-600 mb-1">最大含み損（%）</label>
          <input type="number" step="0.1" value={maxLossPct} onChange={(e) => setMaxLossPct(e.target.value)}
            placeholder="-1.5"
            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" />
        </div>
      </div>
      <div>
        ...
        <textarea value={resultNote} onChange={(e) => setResultNote(e.target.value)}
          rows={2} placeholder="エントリータイミングが良かった"
          className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" />
      </div>
