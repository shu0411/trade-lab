...
export default function EntryNew() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!ticker || !tickerName) {
      setError('銘柄コードと銘柄名を入力してください');
      return;
    }
    if (type === 'entry' && !entryPrice) {
      setError('エントリー価格を入力してください');
      return;
    }
          ...
          <div className="space-y-4 bg-blue-50 rounded-xl p-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <Field label="利確目標（%）">
                ...
                />
                {targetPrice && (
                  <p className="text-xs text-green-600 mt-1">→ {targetPrice.toLocaleString()}円</p>
                )}
              </Field>
              <Field label="損切目標（%）">
                <input
                ...
                />
                {stopPrice && (
                  <p className="text-xs text-red-500 mt-1">→ {stopPrice.toLocaleString()}円</p>
                )}
              </Field>
            </div>
          </div>
