...
export const createEntry = (body: object) =>
  api.post<Entry>('/entries', body).then((r) => r.data);
...
export const getPresignedUrl = (filename: string, contentType: string) =>
  api.post<{ url: string; key: string }>('/upload/presigned', { filename, contentType }).then((r) => r.data);

export const getSignedReadUrl = (key: string) =>
  api.get<{ url: string }>(`/upload/signed-url/${key}`).then((r) => r.data);

export const getSummary = () =>
  api.get<Summary>('/analysis/summary').then((r) => r.data);

export const getPatternStats = () =>
  api.get<PatternStat[]>('/analysis/patterns').then((r) => r.data);

export const getReasonStats = () =>
  api.get<ReasonStat[]>('/analysis/reasons').then((r) => r.data);
