...
def _seed(store, *, type="entry", pattern="押し目", status="open", result=None, reasons=None):
    payload = {**BASE, "type": type, "pattern": pattern, "reasons": reasons or ["25日線反発"]}
    if type == "pass":
        payload.update({"entryPrice": None, "targetPct": None, "stopPct": None})
    item = entry_to_dynamo(EntryCreate(**payload))
    if status == "closed":
        item.update({"status": "closed", "result": result, "closedAt": "2026-06-25T00:00:00"})
    store.create(item)
    return item
...
def test_summary_empty(client):
    ...
    assert data == {"total": 0, "open": 0, "closed": 0, "successRate": None, "passCount": 0}
